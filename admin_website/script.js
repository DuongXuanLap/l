...
document.addEventListener('DOMContentLoaded', async function() {
    initializeApp();
    await loadRealData(); // Try to load real data first
...
// Initialize pattern analysis chart
function initializePatternChart() {
    ...
    new Chart(ctx, {
        type: 'radar',
        data: {
            labels: ['Punctuality', 'Consistency', 'Overtime', 'Remote Work', 'Team Collaboration'],
            datasets: [{
...
function editGeofence(geofenceId) {
    showToast(`Editing geofence: ${geofenceId}`, 'info');
}

function deleteGeofence(geofenceId) {
    if (confirm('Are you sure you want to delete this geofence?')) {
        showToast(`Geofence ${geofenceId} deleted`, 'success');
    }
}
...
async function loadRealData() {
    try {
        ...
        // Load check-ins
        const checkinsData = await apiCall('checkins');
...
function updateStatsFromAPI(stats) {
    ...
    document.getElementById('anomalies').textContent = stats.anomalies_detected || 0;
    ...
    document.getElementById('remoteCount').textContent = stats.remote_count || 0;
}

// Enhanced employee management with real API calls
async function submitAddEmployeeReal() {
...
// Enhanced check-ins loading with real API
async function loadCheckinsReal() {
    try {
        const data = await apiCall('checkins');
        checkins = data.checkins || [];
        loadCheckins();
    } catch (error) {
        console.log('Using sample check-ins data');
        loadCheckins();
    }
}

// Enhanced employees loading with real API
async function loadEmployeesReal() {
    try {
        const data = await apiCall('employees');
        employees = data.employees || [];
        loadEmployees();
    } catch (error) {
        console.log('Using sample employees data');
        loadEmployees();
    }
}
